...
macron <- corpus(c(macron12march,macron16march))
macron
...
boris16mars <- tibble(txt = boris16mars)

boris16mars.fr<- boris16mars %>% unnest_tokens(word, txt) %>%
...
corpus_boris_0 <- tokens(corpus_boris, remove_numbers = TRUE, remove_punct = TRUE, remove_symbols = TRUE, remove_separators = TRUE)

## Lemmatization
corpus_boris_1 <- tokens_replace(corpus_boris_0, pattern=hash_lemmas$token, replacement = hash_lemmas$lemma)

## Cleaning
corpus_boris_2=corpus_boris_1 %>% 
  tokens_tolower() %>% 
  tokens_remove(stopwords("english")) 

## Document-Term Matrix DTM
corpus_boris.dfm <- dfm(corpus_boris_2)
...
N.boris <- ntoken(corpus_boris_2)
V.boris <- ntype(corpus_boris_2)
TTR.boris <- V.boris/N.boris
TTR.boris ###the text is quite poor, as TTR is of 0.4
...
## Lexical Divesity Token Type Ratio TTR
N.macron <- ntoken(corpus_macron_2)
V.macron <- ntype(corpus_macron_2)
...
```{r}
boris.fr<- corpus_boris_0 %>% group_by(text) %>% count(word,sort = TRUE) %>% ungroup()
